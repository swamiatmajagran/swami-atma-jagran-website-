...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!paymentResponse.ok) return res.status(400).json({ error: payment?.error?.description || 'Unable to verify payment status.' });
    if (payment.status !== 'captured') return res.status(400).json({ error: `Payment status is ${payment.status}, not captured.` });

    return res.status(200).json({ ok: true, status: 'PAYMENT_VERIFIED', paymentId: razorpay_payment_id, orderId, product: product || payment.description || '' });
  } catch (e) {
